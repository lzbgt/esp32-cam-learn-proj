...
static esp_err_t lwip_err_to_esp_err(esp_err_t e)
{
    switch (e) {
...
static esp_err_t http_send_response_headers(http_context_t http_ctx)
{
    ...
    esp_err_t err = netconn_write(http_ctx->conn, headers_buf, strlen(headers_buf), NETCONN_COPY);
    free(headers_buf);
...
esp_err_t http_response_write(http_context_t http_ctx, const http_buffer_t* buffer)
{
    ...
    esp_err_t rc = netconn_write(http_ctx->conn, buffer->data, len, flag);
    if (rc != ESP_OK) {
